...
const gradeData = students.reduce((acc, student) => {
      const grade = student.grade_c || student.grade || "Unknown";
...
const percentage = ((grade.score_c || grade.score) / (grade.max_score_c || grade.maxScore)) * 100;
...
{Math.round((attendance.filter(a => (a.status_c || a.status) === "present").length / attendance.length) * 100) || 0}%
            </p>
        ...
        <Card className="hover:shadow-lg transition-all duration-300">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-2xl font-bold text-gray-900">
{Math.round(grades.reduce((sum, g) => sum + ((g.score_c || g.score) / (g.max_score_c || g.maxScore) * 100), 0) / grades.length) || 0}%
            </p>
...
<td className="py-4 px-4 text-gray-700">{classItem.teacher_c || classItem.teacher}</td>
